...
export const analyzeHierarchicalSelection = (
  data: HierarchicalSelectionData
) => {
  const selectedEntries = Object.entries(data).filter(
    ([, info]) => info.selected
  );
  const primarySelections = selectedEntries.filter(
    ([, info]) => info.level === 'primary'
  );
  const secondarySelections = selectedEntries.filter(
    ([, info]) => info.level === 'secondary'
  );
  const tertiarySelections = selectedEntries.filter(
    ([, info]) => info.level === 'tertiary'
  );
  ...
  switch (config.key) {
    case 'customization_duration': {
      ...
      if (typeof value === 'number') {
        ...
        const workingPercent = Math.round(
          (durationData.workingTime / durationData.totalDuration) * 100
        );
      ...
      if (Array.isArray(value)) {
        ...
        if (
          ...
          (!equipmentData.location &&
            equipmentData.contexts.length === 0 &&
            equipmentData.specificEquipment.length === 0 &&
            (!equipmentData.weights ||
              Object.keys(equipmentData.weights).length === 0))
        ) {
          return null;
        }
        ...
        const hasWeights =
          equipmentData.weights &&
          Object.keys(equipmentData.weights).some(
            (key) => equipmentData.weights![key].length > 0
          );
        ...
        if (equipmentData.location && equipmentData.contexts.length === 0) {
          ...
          return (
            locationLabels[
              equipmentData.location as keyof typeof locationLabels
            ] || equipmentData.location
          );
        }
        ...
        if (equipmentData.contexts.length === 1) {
          return hasWeights
            ? `${equipmentData.contexts[0]} + weights`
            : equipmentData.contexts[0];
        }

        if (equipmentData.contexts.length > 1) {
          return hasWeights
            ? `${equipmentData.contexts.length} types + weights`
            : `${equipmentData.contexts.length} types`;
        }

        if (equipmentData.specificEquipment.length > 0) {
          return hasWeights
            ? `${equipmentData.specificEquipment.length} items + weights`
            : `${equipmentData.specificEquipment.length} items`;
        }
    ...
    case 'customization_soreness': {
      ...
      const selectedEntries = Object.entries(categoryData).filter(
        ([, info]) => info.selected
      );
    ...
    case 'customization_include': {
      ...
      if (
        typeof value === 'object' &&
        value !== null &&
        'customExercises' in value
      ) {
        const exerciseData = value as {
...
export const getCustomizationDescription = (
  key: keyof typeof customizationDescriptions
): string => {

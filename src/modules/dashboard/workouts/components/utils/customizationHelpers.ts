...
export const formatSelectionSummary = (
  ...
  if (displayItems.length === 1) {
    ...
    return item.length > truncateAt
      ? `${item.substring(0, truncateAt)}...`
      : item;
  }
...
export const formatHierarchicalSummary = (
  data: Record<
    string,
    { selected: boolean; label: string; level: string; parentKey?: string }
  >,
  options: SelectionSummaryOptions = {}
): string | null => {
  ...
  const byLevel = selectedEntries.reduce(
    (acc, [, info]) => {
...
export const handleMultipleSelection = <T>(
  ...
  const { allowEmpty = true, maxSelections, onSelectionChange } = options;
...
export const handleCategorySelection = <
  T extends { category: string; value: string },
>(
  currentSelections: T[],
  categoryItems: T[],
  toggleItem: T,
  ...
  if (selectAllInCategory) {
    ...
    if (allSelected) {
      ...
      return currentSelections.filter(
        (sel) => !categoryItemValues.includes(sel.value)
      );
    } else {
...
export const validateTimeAllocation = (
  ...
  const structurePercentage =
    ((warmUpMinutes + coolDownMinutes) / totalMinutes) * 100;
  ...
  if (workingTime < minWorkingTime) {
    result.isValid = false;
    result.errors.push(
      `Working time (${workingTime} min) is below minimum (${minWorkingTime} min)`
    );
  }
  ...
  if (warmUpMinutes + coolDownMinutes >= totalMinutes) {
    result.isValid = false;
    result.errors.push(
      'Preparation time cannot equal or exceed total duration'
    );
  }
  ...
  if (workingPercentage < 50) {
    result.recommendations!.push(
      'Very low working time - consider shorter preparation'
    );
  } else if (workingPercentage > 95) {
    result.recommendations!.push(
      'Consider adding warm-up/cool-down for injury prevention'
    );
  }
...
export const formatTimeRange = (
  ...
  const minFormatted = formatTimeDisplay(minMinutes, {
    ...options,
    showUnits: false,
  });
...
export const calculatePercentage = (
  ...
  const constrainedPercentage = Math.max(
    minValue,
    Math.min(maxValue, rawPercentage)
  );

  // Round to specified precision
  const percentage =
    Math.round(constrainedPercentage * Math.pow(10, precision)) /
...
export const getGridItemClass = (
  ...
  if (disabled) {
    ...
    stateClass =
      'bg-base-100 border-base-300 text-base-content hover:border-base-400 hover:bg-base-200 cursor-pointer';
  }

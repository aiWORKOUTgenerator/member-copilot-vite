...
export type WorkoutFormatValue = 
  | 'straight_sets' | 'pyramid_sets' | 'cluster_sets' | 'rest_pause'
  | 'competition_style' | 'block_periodization' | 'conjugate_method'
  | 'heavy_light' | 'dup' | 'push_pull_legs'
  | 'straight_sets_hypertrophy' | 'supersets' | 'drop_sets' | 'giant_sets'
  | 'progressive_overload' | 'time_under_tension' | 'isometric_holds'
  | 'tabata' | 'emom' | 'circuit_training' | 'sprint_intervals'
  | 'steady_state' | 'tempo_intervals' | 'fartlek'
  | 'metabolic_circuits' | 'cardio_strength_fusion' | 'liss_resistance'
  | 'movement_patterns' | 'sport_specific' | 'crossfit_style'
  | 'dynamic_stretching' | 'static_stretching' | 'pnf_stretching' | 'yoga_flow'
  | 'gentle_flow' | 'foam_rolling' | 'breathwork';

// Type guard for workout format values
export function isWorkoutFormatValue(value: unknown): value is WorkoutFormatValue {
...
export function isWorkoutFocusConfigurationData(value: unknown): value is WorkoutFocusConfigurationData {
  ...
  if (
    ...
    !['beginner', 'intermediate', 'advanced', 'all-levels'].includes(metadata.experience) ||
    !Array.isArray(metadata.duration_compatibility) ||
    !metadata.duration_compatibility.every((d: number) => typeof d === 'number') ||
    !['strength_power', 'muscle_building', 'conditioning_cardio', 'functional_recovery'].includes(metadata.category)
  ) {
    return false;
  }

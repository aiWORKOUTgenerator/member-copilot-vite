...
import {
  formatWorkoutSelection,
  getRatingLabel,
} from './utils/selectionFormatter';
...
export default function WorkoutSummary({ options, mode }: WorkoutSummaryProps) {
  ...
  const extractIndividualSelections = (
    key: string,
    value: unknown
  ): string[] => {
    if (!value) return [];
    ...
    switch (key) {
      ...
      case 'customization_areas': {
        if (Array.isArray(value)) {
          ...
          const selectedEntries = Object.entries(hierarchicalData).filter(
            ([, info]) => info.selected
          );
  ...
  const renderCustomizationSummary = (key: string, value: unknown) => {
    ...
    return (
      <div
        key={key}
        className="flex items-start py-2 border-b border-base-200 last:border-b-0"
      >
  ...
  const renderSummaryGroup = (groupName: string, keys: readonly string[]) => {
    ...
    return (
      <div key={groupName} className="mb-4">
        <h5 className="text-xs font-semibold text-base-content/60 mb-2 uppercase tracking-wide">
          {groupName}
        </h5>
        <div className="bg-base-50 rounded-lg p-3">
          {groupOptions.map(([key, value]) =>
            renderCustomizationSummary(key, value)
          )}
        </div>
      </div>
    );
            ...
            <div className="mockup-code text-xs">
              {selectedOptions.map(([key, value]) => {
                ...
                const formatted = config
                  ? formatWorkoutSelection(config, value)
                  : String(value);
                return (
                  formatted && (

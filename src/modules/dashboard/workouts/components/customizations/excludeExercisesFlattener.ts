...
const EXERCISE_RISK_CLASSIFICATIONS = {
  ...
  fatigue_risk: [
    'high-rep complexes',
    'planks over 1 minute',
    'excessive foam rolling',
    'unsupported wall sits',
  ],
};
...
function exerciseMatchesRiskClassification(
  exercise: string,
...
export function flattenExcludeExercisesData(
  data: string | undefined
): EnhancedExcludeExercisesFlat {
  ...
  Object.entries(EXERCISE_EXCLUSION_CATEGORIES).forEach(
    ([category, exercises]) => {
      ...
      switch (category) {
        case 'Full Body':
          ...
          result.exclusions_excludes_burpees = checkExerciseExcluded(
            'burpees',
            allExclusions
          );
          result.exclusions_excludes_man_makers = checkExerciseExcluded(
            'man makers',
            allExclusions
          );
          result.exclusions_excludes_jumping_jacks = checkExerciseExcluded(
            'jumping jacks',
            allExclusions
          );
          result.exclusions_excludes_thrusters = checkExerciseExcluded(
            'thrusters',
            allExclusions
          );
          result.exclusions_excludes_wall_balls = checkExerciseExcluded(
            'wall balls',
            allExclusions
          );
          result.exclusions_excludes_box_jumps = checkExerciseExcluded(
            'box jumps',
            allExclusions
          );
          result.exclusions_excludes_mountain_climbers = checkExerciseExcluded(
            'mountain climbers',
            allExclusions
          );
          result.exclusions_excludes_high_rep_complexes = checkExerciseExcluded(
            'high-rep complexes',
            allExclusions
          );
          break;
        case 'Lower Body':
          ...
          result.exclusions_excludes_deep_jump_squats = checkExerciseExcluded(
            'deep jump squats',
            allExclusions
          );
          result.exclusions_excludes_sissy_squats = checkExerciseExcluded(
            'sissy squats',
            allExclusions
          );
          result.exclusions_excludes_barbell_back_squats =
            checkExerciseExcluded('barbell back squats', allExclusions);
          result.exclusions_excludes_pistol_squats = checkExerciseExcluded(
            'pistol squats',
            allExclusions
          );
          result.exclusions_excludes_hack_squats = checkExerciseExcluded(
            'hack squats',
            allExclusions
          );
          result.exclusions_excludes_walking_lunges = checkExerciseExcluded(
            'walking lunges',
            allExclusions
          );
          result.exclusions_excludes_good_mornings = checkExerciseExcluded(
            'good mornings',
            allExclusions
          );
          result.exclusions_excludes_box_step_ups = checkExerciseExcluded(
            'box step-ups',
            allExclusions
          );
          break;
        case 'Upper Body':
          ...
          result.exclusions_excludes_behind_neck_press = checkExerciseExcluded(
            'behind-the-neck press',
            allExclusions
          );
          result.exclusions_excludes_kipping_pull_ups = checkExerciseExcluded(
            'kipping pull-ups',
            allExclusions
          );
          result.exclusions_excludes_heavy_upright_rows = checkExerciseExcluded(
            'heavy upright rows',
            allExclusions
          );
          result.exclusions_excludes_dips_straight_bars = checkExerciseExcluded(
            'dips on straight bars',
            allExclusions
          );
          result.exclusions_excludes_push_press = checkExerciseExcluded(
            'push press',
            allExclusions
          );
          result.exclusions_excludes_arnold_press = checkExerciseExcluded(
            'arnold press',
            allExclusions
          );
          result.exclusions_excludes_barbell_bench_press =
            checkExerciseExcluded('barbell bench press', allExclusions);
          result.exclusions_excludes_heavy_overhead_press =
            checkExerciseExcluded('heavy overhead press', allExclusions);
          break;
        case 'Core & Stability':
          result.exclusions_has_core_stability = categoryHasExclusions;
          result.exclusions_excludes_sit_ups = checkExerciseExcluded(
            'sit-ups',
            allExclusions
          );
          result.exclusions_excludes_superman_holds = checkExerciseExcluded(
            'superman holds',
            allExclusions
          );
          result.exclusions_excludes_windshield_wipers = checkExerciseExcluded(
            'windshield wipers',
            allExclusions
          );
          result.exclusions_excludes_crunches = checkExerciseExcluded(
            'crunches',
            allExclusions
          );
          result.exclusions_excludes_toe_to_bar = checkExerciseExcluded(
            'toe-to-bar',
            allExclusions
          );
          result.exclusions_excludes_v_ups = checkExerciseExcluded(
            'v-ups',
            allExclusions
          );
          result.exclusions_excludes_weighted_russian_twists =
            checkExerciseExcluded('weighted russian twists', allExclusions);
          result.exclusions_excludes_long_planks = checkExerciseExcluded(
            'planks over 1 minute',
            allExclusions
          );
          break;
        case 'Mobility & Corrective / Recovery':
          ...
          result.exclusions_excludes_standing_toe_touches =
            checkExerciseExcluded('standing toe touches', allExclusions);
          result.exclusions_excludes_seated_forward_fold =
            checkExerciseExcluded('seated forward fold', allExclusions);
          result.exclusions_excludes_unsupported_wall_sits =
            checkExerciseExcluded('unsupported wall sits', allExclusions);
          result.exclusions_excludes_hip_circles = checkExerciseExcluded(
            'hip circles',
            allExclusions
          );
          result.exclusions_excludes_excessive_foam_rolling =
            checkExerciseExcluded('excessive foam rolling', allExclusions);
          result.exclusions_excludes_deep_squat_hold = checkExerciseExcluded(
            'deep squat hold',
            allExclusions
          );
          result.exclusions_excludes_overhead_shoulder_cars =
            checkExerciseExcluded('overhead shoulder cars', allExclusions);
          result.exclusions_excludes_ballistic_arm_swings =
            checkExerciseExcluded('ballistic arm swings', allExclusions);
          break;
      }
    }
  );
  ...
  result.exclusions_avoids_high_injury_risk = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.high_injury_risk
    )
  );
  result.exclusions_avoids_joint_stress = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.joint_stress
    )
  );
  result.exclusions_avoids_skill_demanding = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.skill_demanding
    )
  );
  result.exclusions_avoids_equipment_dependent = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.equipment_dependent
    )
  );
  result.exclusions_avoids_spine_risky = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.spine_risky
    )
  );
  result.exclusions_avoids_shoulder_risky = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.shoulder_risky
    )
  );
  result.exclusions_avoids_beginner_unsafe = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.beginner_unsafe
    )
  );
  result.exclusions_avoids_fatigue_risk = allExclusions.some((ex) =>
    exerciseMatchesRiskClassification(
      ex,
      EXERCISE_RISK_CLASSIFICATIONS.fatigue_risk
    )
  );

  // Analyze body region protection
  result.exclusions_protects_lower_back =
    result.exclusions_excludes_sit_ups ||
    result.exclusions_excludes_good_mornings ||
    result.exclusions_excludes_seated_forward_fold;
  result.exclusions_protects_shoulders =
    result.exclusions_excludes_behind_neck_press ||
    result.exclusions_excludes_wall_balls ||
    result.exclusions_excludes_push_press;
  result.exclusions_protects_knees =
    result.exclusions_excludes_deep_jump_squats ||
    result.exclusions_excludes_sissy_squats ||
    result.exclusions_excludes_box_jumps;
  result.exclusions_protects_wrists =
    result.exclusions_excludes_mountain_climbers ||
    allExclusions.some((ex) => ex.toLowerCase().includes('wrist'));
  result.exclusions_protects_neck =
    result.exclusions_excludes_crunches ||
    result.exclusions_excludes_behind_neck_press;
  result.exclusions_protects_spine =
    result.exclusions_excludes_sit_ups ||
    result.exclusions_excludes_superman_holds ||
    result.exclusions_excludes_windshield_wipers;
  ...
  result.exclusions_safety_priority_score = Math.min(
    10,
    riskScore + limitationScore
  );

  // Set implications based on analysis
  result.exclusions_indicates_injury_history =
    result.exclusions_risk_level_score >= 5;
  result.exclusions_indicates_beginner_level =
    result.exclusions_avoids_beginner_unsafe ||
    result.exclusions_avoids_skill_demanding;
  result.exclusions_indicates_equipment_limitations =
    result.exclusions_avoids_equipment_dependent;
  result.exclusions_indicates_skill_limitations =
    result.exclusions_avoids_skill_demanding;

  // Set training adaptations needed
  result.exclusions_requires_low_impact =
    result.exclusions_avoids_joint_stress || result.exclusions_protects_knees;
  result.exclusions_requires_joint_friendly =
    result.exclusions_protects_lower_back ||
    result.exclusions_protects_shoulders;
  result.exclusions_requires_simple_movements =
    result.exclusions_avoids_skill_demanding;
  result.exclusions_requires_bodyweight_only =
    result.exclusions_avoids_equipment_dependent;

  // Set recommendations
  result.exclusions_recommend_medical_clearance =
    result.exclusions_risk_level_score >= 7;
  result.exclusions_recommend_professional_guidance =
    result.exclusions_indicates_injury_history ||
    result.exclusions_safety_priority_score >= 6;
  result.exclusions_recommend_modified_movements =
    result.exclusions_total_count >= 5;
  result.exclusions_recommend_alternative_exercises =
    result.exclusions_category_count >= 2;

  // Set planning guidance
  result.exclusions_prioritize_safety = result.exclusions_risk_level_score >= 4;
  result.exclusions_prioritize_modifications =
    result.exclusions_limitation_score >= 3;
  result.exclusions_prioritize_alternatives =
    result.exclusions_category_count >= 3;
  result.exclusions_prioritize_pain_free =
    result.exclusions_indicates_injury_history;
...
function checkExerciseExcluded(
  exerciseName: string,
...
export function getExcludeExercisesSummary(
  flattened: EnhancedExcludeExercisesFlat
): string {
  const parts = [];
  ...
  // Risk analysis
  if (flattened.exclusions_avoids_high_injury_risk)
    parts.push('Avoiding high-risk exercises');
  if (flattened.exclusions_protects_spine) parts.push('Protecting spine');
  if (flattened.exclusions_protects_shoulders)
    parts.push('Protecting shoulders');
  if (flattened.exclusions_protects_lower_back)
    parts.push('Protecting lower back');

  // Safety implications
  if (flattened.exclusions_indicates_injury_history)
    parts.push('Possible injury history');
  if (flattened.exclusions_indicates_beginner_level)
    parts.push('Beginner-level safety');
  if (flattened.exclusions_indicates_equipment_limitations)
    parts.push('Equipment limitations');

  // Recommendations
  if (flattened.exclusions_recommend_medical_clearance)
    parts.push('Medical clearance recommended');
  else if (flattened.exclusions_recommend_professional_guidance)
    parts.push('Professional guidance recommended');
  else if (flattened.exclusions_prioritize_safety)
    parts.push('Safety-first approach');

  // Training adaptations
  if (flattened.exclusions_requires_low_impact) parts.push('Low-impact focus');
  if (flattened.exclusions_requires_simple_movements)
    parts.push('Simple movements preferred');

...
const WARMUP_PRESETS = [
  {
    label: 'Minimal (1 min)',
    value: 1,
    description: 'Ultra-brief activation for micro sessions',
  },
  {
    label: 'Quick (2 min)',
    value: 2,
    description: 'Basic activation for short sessions',
  },
  {
    label: 'Brief (3 min)',
    value: 3,
    description: 'Light preparation for express workouts',
  },
  {
    label: 'Standard (5 min)',
    value: 5,
    description: 'Recommended preparation for most workouts',
  },
  {
    label: 'Extended (8 min)',
    value: 8,
    description: 'Thorough preparation for intense sessions',
  },
  {
    label: 'Comprehensive (10 min)',
    value: 10,
    description: 'Complete preparation for long workouts',
  },
  {
    label: 'Full Prep (15 min)',
    value: 15,
    description: 'Maximum preparation for extended training',
  },
];
...
const COOLDOWN_PRESETS = [
  {
    label: 'Quick (3 min)',
    value: 3,
    description: 'Basic recovery for short sessions',
  },
  {
    label: 'Brief (5 min)',
    value: 5,
    description: 'Standard recovery for most workouts',
  },
  {
    label: 'Extended (7 min)',
    value: 7,
    description: 'Enhanced recovery for intense sessions',
  },
  {
    label: 'Comprehensive (10 min)',
    value: 10,
    description: 'Thorough recovery for long workouts',
  },
  {
    label: 'Recovery Focus (15 min)',
    value: 15,
    description: 'Maximum recovery for extended training',
  },
];
...
const validateTimeAllocation = (
  total: number,
  warmUp: number,
  coolDown: number
): DurationConfigurationData['validation'] => {
  ...
  if (warmUp + coolDown >= total) {
    validation.isValid = false;
    validation.errors!.push(
      'Preparation time cannot equal or exceed total workout duration'
    );
  }
  ...
  if (total >= 90 && coolDown < 8) {
    validation.warnings!.push(
      'Extended sessions benefit from longer recovery periods'
    );
  }
  ...
  if (workingPercentage < 50) {
    validation.warnings!.push(
      'Very low working time percentage - consider shorter preparation'
    );
  } else if (workingPercentage > 95) {
    validation.warnings!.push(
      'Consider adding warm-up or cool-down for injury prevention'
    );
  }
...
const calculateSessionEfficiency = (
  total: number,
  working: number
): {
  percentage: number;
  ...
  if (percentage >= 85) {
    ...
    return {
      ...
      rating: 'moderate',
      recommendation:
        'Consider reducing preparation time for more active training',
    };
  } else {
    return {
      percentage,
      rating: 'poor',
      recommendation:
        'Too much preparation time - consider shorter warm-up/cool-down',
    };
  }
};

const generateOptimalSuggestions = (
  ...
  if (totalDuration <= 10) {
    return {
      ...
      coolDown: 2,
      reasoning:
        'Micro sessions need minimal preparation to maximize working time',
    };
...
const generateSmartDescription = (
  ...
  const baseDescription =
    preset?.description || `${totalDuration} minute workout session`;
...
): DurationConfigurationData['configuration'] => {
...
const WorkoutDurationCustomization = memo(
  function WorkoutDurationCustomization({
    value,
    onChange,
    disabled = false,
    error,
    ...
    const [showStructureOptions, setShowStructureOptions] = useState(
      existingData
        ? existingData.warmUp.included || existingData.coolDown.included
        : false
    );

    // Smart calculation and validation integration
    const currentWorkingTime = selectedDuration
    ...
    const currentValidation = selectedDuration
    ...
    const sessionEfficiency = selectedDuration
      ? calculateSessionEfficiency(selectedDuration, currentWorkingTime)
      : null;

    const optimalSuggestions = selectedDuration
      ? generateOptimalSuggestions(selectedDuration)
      : null;
    ...
    const buildDurationConfiguration = useCallback(
      (
        ...
        const workingTime = calculateWorkingTime(
          duration,
          warmUp.included ? warmUp.duration : 0,
          coolDown.included ? coolDown.duration : 0
        );
        const configuration = determineConfiguration(
          warmUp.included,
          coolDown.included
        );
        const validation = validateTimeAllocation(
          duration,
          warmUp.included ? warmUp.duration : 0,
          coolDown.included ? coolDown.duration : 0
        );
        ...
        return {
          ...
          warmUp: {
            ...
            percentage: warmUp.included
              ? Math.round((warmUp.duration / duration) * 100)
              : 0,
          },
          coolDown: {
            included: coolDown.included,
            duration: coolDown.included ? coolDown.duration : 0,
            percentage: coolDown.included
              ? Math.round((coolDown.duration / duration) * 100)
              : 0,
          },
          workingTime,
          configuration,
    ...
    const emitChange = useCallback(
      (
        ...
        const configurationData = buildDurationConfiguration(
          duration,
          warmUp,
          coolDown
        );
    ...
    const updateCoolDownDuration = useCallback(
      (duration: number) => {
        if (!selectedDuration) return;
    ...
    const applyOptimalSuggestions = useCallback(() => {
      ...
      const newWarmUpConfig = {
        included: true,
        duration: optimalSuggestions.warmUp,
      };
      const newCoolDownConfig = {
        included: true,
        duration: optimalSuggestions.coolDown,
      };
    ...
    const groupedPresets = useMemo(() => {
      return DURATION_PRESETS.reduce(
        (acc, preset) => {
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {presets.map((preset) => (
                  <button
                    ...
                    onClick={() =>
                      !disabled && handleDurationSelection(preset.value)
                    }
                    disabled={disabled}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-base-content/80 mb-2">
                    For your {selectedDuration}-minute session, we recommend
                    adding {optimalSuggestions.warmUp}min warm-up +{' '}
                    {optimalSuggestions.coolDown}min cool-down.
                  </p>
          ...
          <div className="border border-base-300 rounded-lg p-4">
            <div className="mb-3">
              ...
              <p className="text-sm text-base-content/80 mb-3">
                Add warm-up and cool-down phases to your {selectedDuration}
                -minute session:
              </p>
            </div>
                ...
                {warmUpConfig.included && (
                  <div className="text-right">
                    <span className="text-sm font-medium text-primary">
                      {warmUpConfig.duration} min
                    </span>
                    <p className="text-xs text-base-content/60">
                      {Math.round(
                        (warmUpConfig.duration / selectedDuration) * 100
                      )}
                      % of session
                    </p>
                  </div>
                ...
                {coolDownConfig.included && (
                  <div className="text-right">
                    <span className="text-sm font-medium text-secondary">
                      {coolDownConfig.duration} min
                    </span>
                    <p className="text-xs text-base-content/60">
                      {Math.round(
                        (coolDownConfig.duration / selectedDuration) * 100
                      )}
                      % of session
                    </p>
                  </div>
        ...
        {warmUpConfig.included && (
          <div className="bg-base-100 border border-base-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {WARMUP_PRESETS.filter(
                (preset) => preset.value <= selectedDuration! - 5
              ).map((preset) => (
                <button
                  key={preset.value}
                  type="button"
                  ...
                  onClick={() =>
                    !disabled && updateWarmUpDuration(preset.value)
                  }
                  disabled={disabled}
                  title={preset.description}
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {COOLDOWN_PRESETS.filter(
                ...
                <button
                  ...
                  onClick={() =>
                    !disabled && updateCoolDownDuration(preset.value)
                  }
                  disabled={disabled}
                  title={preset.description}
                >
              ...
              <div className="ml-4 text-right">
                <div
                  className={`text-xs font-medium px-2 py-1 rounded-full ${
                    sessionEfficiency.rating === 'excellent'
                      ? 'bg-success text-success-content'
                      : sessionEfficiency.rating === 'good'
                        ? 'bg-info text-info-content'
                        : sessionEfficiency.rating === 'moderate'
            ...
            <div className="grid grid-cols-2 gap-4 text-xs text-base-content/70">
              <div>
                ...
                <div className="mt-1 space-y-1">
                  <div>
                    Working Time: <strong>{currentWorkingTime} min</strong> (
                    {sessionEfficiency.percentage}%)
                  </div>
                  {warmUpConfig.included && (
                    <div>
                      • Warm-Up: {warmUpConfig.duration} min (
                      {Math.round(
                        (warmUpConfig.duration / selectedDuration) * 100
                      )}
                      %)
                  ...
                  {coolDownConfig.included && (
                    <div>
                      • Cool-Down: {coolDownConfig.duration} min (
                      {Math.round(
                        (coolDownConfig.duration / selectedDuration) * 100
                      )}
                      %)
              ...
              <div>
                ...
                <div className="mt-1">
                  <div className="text-xs">
                    {sessionEfficiency.recommendation}
                  </div>
                </div>
              </div>
            </div>

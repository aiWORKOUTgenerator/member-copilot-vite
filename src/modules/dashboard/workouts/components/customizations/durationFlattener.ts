...
export function flattenDurationData(data: DurationConfigurationData | number | undefined): EnhancedDurationFlat {
...
function flattenLegacyDuration(totalMinutes: number): EnhancedDurationFlat {
  ...
  result.duration_optimal_for_location_home = totalMinutes >= 20 && totalMinutes <= 45;
...
function flattenModernDuration(data: DurationConfigurationData): EnhancedDurationFlat {
  const result = createEmptyDurationFlattened();
  
  // CORE DURATION DATA
  result.duration_total_minutes = data.totalDuration;
  result.duration_working_minutes = data.workingTime;
  result.duration_structure_minutes = data.warmUp.duration + data.coolDown.duration;
  ...
  result.duration_warmup_standard = data.warmUp.duration >= 3 && data.warmUp.duration <= 7;
  ...
  result.duration_cooldown_standard = data.coolDown.duration >= 4 && data.coolDown.duration <= 7;
  result.duration_cooldown_extended = data.coolDown.duration >= 8;
  
  // SESSION CATEGORIZATION
  result.duration_category_micro = data.totalDuration <= 15;
  result.duration_category_short = data.totalDuration > 15 && data.totalDuration <= 30;
  result.duration_category_standard = data.totalDuration > 30 && data.totalDuration <= 60;
  result.duration_category_extended = data.totalDuration > 60 && data.totalDuration <= 90;
  ...
  result.duration_config_full_structure = data.configuration === 'full-structure';
  
  // EFFICIENCY METRICS
  result.duration_working_percentage = Math.round((data.workingTime / data.totalDuration) * 100);
  result.duration_structure_percentage = Math.round((result.duration_structure_minutes / data.totalDuration) * 100);
  result.duration_efficiency_excellent = result.duration_working_percentage >= 85;
  result.duration_efficiency_good = result.duration_working_percentage >= 70 && result.duration_working_percentage < 85;
  result.duration_efficiency_moderate = result.duration_working_percentage >= 50 && result.duration_working_percentage < 70;
  ...
  result.duration_balanced_structure = Math.abs(warmUpDuration - coolDownDuration) <= 2;
  ...
  if (data.validation) {
    ...
    result.duration_validation_has_warnings = (data.validation.warnings?.length || 0) > 0;
    result.duration_validation_has_errors = (data.validation.errors?.length || 0) > 0;
    result.duration_validation_warning_count = data.validation.warnings?.length || 0;
    result.duration_validation_error_count = data.validation.errors?.length || 0;
  }
  
  // RECOMMENDATION FLAGS
  result.duration_recommended_for_beginners = (
    data.totalDuration >= 20 && data.totalDuration <= 45 &&
    data.warmUp.included && data.coolDown.included
  );
  ...
  result.duration_injury_prevention_optimized = (
    data.warmUp.included && data.coolDown.included &&
    data.warmUp.duration >= 3 && data.coolDown.duration >= 3
  );
  ...
  result.duration_suitable_hiit = (
    data.totalDuration >= 15 && data.totalDuration <= 45 &&
    data.workingTime >= 10
  );
  ...
  // SMART ANALYTICS
  result.duration_optimal_for_location_home = data.totalDuration >= 20 && data.totalDuration <= 45;
  result.duration_optimal_for_location_gym = data.totalDuration >= 45 && data.totalDuration <= 75;
  result.duration_optimal_for_location_quick = data.totalDuration <= 20;
  result.duration_prep_to_work_ratio = data.workingTime > 0 ? 
    Math.round((result.duration_structure_minutes / data.workingTime) * 100) / 100 : 0;
...
function calculateRecoveryCapacity(data: DurationConfigurationData): number {
  ...
  const structurePercentage = ((data.warmUp.duration + data.coolDown.duration) / data.totalDuration) * 100;
...
export function getDurationSummary(flattened: EnhancedDurationFlat): string {
  ...
  if (flattened.duration_warmup_included || flattened.duration_cooldown_included) {
  ...
  if (flattened.duration_efficiency_excellent) parts.push('Excellent efficiency');
  else if (flattened.duration_efficiency_good) parts.push('Good efficiency');
  else if (flattened.duration_efficiency_moderate) parts.push('Moderate efficiency');
  
  // Capacity indicators
  if (flattened.duration_intensity_capacity >= 80) parts.push('High intensity capable');
  if (flattened.duration_recovery_capacity >= 80) parts.push('Strong recovery focus');

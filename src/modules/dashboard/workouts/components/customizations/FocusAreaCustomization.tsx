...
const TERTIARY_AREAS = {
  ...
  hamstrings: [
    ...
    {
      label: 'Semimembranosus (Deep Inner Hamstring)',
      value: 'semimembranosus',
    },
  ],
  calves: [
    {
      label: 'Inner Calf (Medial Gastrocnemius)',
      value: 'gastrocnemius_medial',
    },
    {
      label: 'Outer Calf (Lateral Gastrocnemius)',
      value: 'gastrocnemius_lateral',
    },
  ...
  obliques: [
    ...
    {
      label: 'External Obliques (Surface Side Abs)',
      value: 'external_obliques',
    },
  ],
  lower_back: [
...
const getAllSelectableOptions = () => {
  ...
  Object.values(SECONDARY_MUSCLES)
    .flat()
    .forEach((secondary) => {
      options.push({
...
const getChildrenKeys = (
  ...
  if (level === 'primary') {
    return (
      SECONDARY_MUSCLES as Record<string, { value: string; label: string }[]>
    )[value]?.map((secondary) => secondary.value);
  }

  if (level === 'secondary') {
    return (
      TERTIARY_AREAS as Record<string, { value: string; label: string }[]>
    )[value]?.map((tertiary) => tertiary.value);
  }
...
export default memo(function FocusAreaCustomization({
  ...
  const [expandedPrimary, setExpandedPrimary] = useState<Set<string>>(
    new Set()
  );
  const [expandedTertiary, setExpandedTertiary] = useState<Set<string>>(
    new Set()
  );

  // Extract selected areas from hierarchical data for backward compatibility with current UI
  const selectedAreas = Object.keys(hierarchicalData).filter(
    (key) => hierarchicalData[key]?.selected
  );
  ...
  const handleAreaToggle = useCallback(
    (areaValue: string, level?: 'primary' | 'secondary' | 'tertiary') => {
      ...
      if (isSelected) {
        ...
        onChange(
          Object.keys(newHierarchicalData).length > 0
            ? newHierarchicalData
            : undefined
        );
      } else {
      ...
      <div className="space-y-6">
        ...
        {PRIMARY_REGIONS.map((region) => {
          ...
          const secondaryMuscles = (
            SECONDARY_MUSCLES as Record<
              string,
              { value: string; label: string; hasTertiary?: boolean }[]
            >
          )[region.value];
            ...
            secondaryMuscles && (
              ...
              >
                <div className="mb-3">
                  ...
                  <p className="text-sm text-base-content/80 mb-3">
                    Select specific muscle groups within{' '}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {secondaryMuscles.map(
                      (secondary: {
                        ...
                        const isSecondarySelected = selectedAreas.includes(
                          secondary.value
                        );
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              handleSecondaryClick(secondary.value)
                            }
                            disabled={disabled}
                            title={
                              secondary.hasTertiary
                                ? 'Click to select or expand for specific areas'
                                : ''
                            }
                          >
                ...
                {secondaryMuscles.map(
                  (secondary: {
                    ...
                    const isTertiaryExpanded = expandedTertiary.has(
                      secondary.value
                    );
                    const tertiaryAreas = (
                      TERTIARY_AREAS as Record<
                        string,
                        { value: string; label: string }[]
                      >
                    )[secondary.value];
                    ...
                    return (
                      ...
                      tertiaryAreas && (
                        <div
                          key={`${secondary.value}-tertiary`}
                          className="mt-4 bg-base-100 border border-base-200 rounded-lg p-4"
                        >
                          <div className="flex justify-between items-center mb-3">
                            ...
                            <button
                              ...
                              className="btn btn-xs btn-ghost"
                              onClick={() =>
                                toggleTertiaryExpansion(secondary.value)
                              }
                            >
                              ×
                            </button>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {tertiaryAreas.map(
                              (tertiary: { value: string; label: string }) => {
                                const isTertiarySelected =
                                  selectedAreas.includes(tertiary.value);
          ...
          {Object.entries(hierarchicalData)
            ...
            .map(([key, info]) => {
              ...
              if (info.level === 'secondary' && info.parentKey) {
                ...
                const parent = hierarchicalData[info.parentKey];
                const grandparent = parent?.parentKey
                  ? hierarchicalData[parent.parentKey]
                  : null;

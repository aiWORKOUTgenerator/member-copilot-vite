...
const STRESS_LEVELS = [
  ...
  },
  {
    ...
    label: 'Severe',
    description:
      'Overwhelming stress severely impacting function and wellbeing',
  },
];

export default memo(function StressLevelCustomization({
  value,
  onChange,
  disabled = false,
  error,
  ...
  const selectedStressCategories = Object.keys(categoryData).filter(
  ...
  const getStressRecommendation = (category: string, level: number) => {
      ...
      physical: {
        1: (
          <span className="inline-flex items-center space-x-1">
            ...
            <span>Well recovered, great for intense training</span>
          </span>
        ),
        2: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-success">⚡</span>
            <span>Good energy, ready for challenging workouts</span>
          </span>
        ),
        3: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-warning">⚠️</span>
            <span>Moderate fatigue, consider lighter intensity</span>
          </span>
        ),
        4: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-error">🛑</span>
            <span>Significant fatigue, focus on recovery activities</span>
          </span>
        ),
        5: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-error">🚨</span>
            <span>Severe fatigue, rest and recovery essential</span>
          </span>
        ),
      },
      mental_emotional: {
        1: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-success">✓</span>
            <span>Clear focus, excellent for skill-based training</span>
          </span>
        ),
        2: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-success">🧠</span>
            <span>Good mental state, ready for complex movements</span>
          </span>
        ),
        3: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-warning">💭</span>
            <span>Some distraction, prefer routine exercises</span>
          </span>
        ),
        4: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-info">🧘</span>
            <span>High stress, consider mindful movement or yoga</span>
          </span>
        ),
        5: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-info">🤝</span>
            <span>Overwhelmed, seek support and gentle activities</span>
          </span>
        ),
      },
      environmental: {
        1: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-success">✓</span>
            <span>Ideal conditions, any workout type suitable</span>
          </span>
        ),
        2: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-success">🌟</span>
            <span>Good environment, minor adaptations may help</span>
          </span>
        ),
        3: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-warning">🔄</span>
            <span>Some disruption, flexible routine recommended</span>
          </span>
        ),
        4: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-warning">🏠</span>
            <span>Challenging environment, home workouts preferred</span>
          </span>
        ),
        5: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-error">⏸️</span>
            <span>Chaotic conditions, postpone structured training</span>
          </span>
        ),
      },
      systemic: {
        1: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-success">✓</span>
            <span>Fully balanced, optimal training capacity</span>
          </span>
        ),
        2: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-success">⚖️</span>
            <span>Well-managed load, maintain current routine</span>
          </span>
        ),
        3: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-warning">📊</span>
            <span>Noticeable load, monitor recovery closely</span>
          </span>
        ),
        4: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-error">⚠️</span>
            <span>High allostatic load, reduce training volume</span>
          </span>
        ),
        5: (
          <span className="inline-flex items-center space-x-1">
            <span className="text-error">🔴</span>
            <span>Burnout risk, prioritize rest and recovery</span>
          </span>
        ),
      },
    };
    ...
    return (
      recommendations[category as keyof typeof recommendations]?.[
        level as keyof typeof recommendations.physical
      ] || null
    );
  };
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {STRESS_CATEGORIES.map((category) => {
              const isSelected =
                categoryData[category.value]?.selected || false;
        ...
        {selectedStressCategories.length > 0 && (
          <div className="mt-6 space-y-4">
            ...
            {selectedStressCategories.map((categoryValue) => {
              const category = STRESS_CATEGORIES.find(
                (c) => c.value === categoryValue
              );
              const selectedLevel = categoryData[categoryValue]?.rating;
              const selectedStressLevel = STRESS_LEVELS.find(
              ...
              return (
                ...
                >
                  <div className="mb-3">
                    <h5 className="font-medium text-base-content mb-1">
                      {category?.label}
                    </h5>
                    ...
                    <div
                      role="radiogroup"
                      aria-labelledby={`stress-${categoryValue}-label`}
                    >
                      <p
                        id={`stress-${categoryValue}-label`}
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-1">
            {selectedStressCategories.map((categoryValue) => {
              ...
              const levelLabel = STRESS_LEVELS.find(
                (l) => l.value === level
              )?.label;

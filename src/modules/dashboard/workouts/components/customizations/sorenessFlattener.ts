...
export function flattenSorenessData(data: CategoryRatingData | undefined): EnhancedSorenessFlat {
  ...
  bodyParts.forEach(bodyPart => {
    ...
    // Set presence flags
    switch (bodyPart) {
      case 'neck': result.soreness_has_neck = true; break;
      case 'shoulders': result.soreness_has_shoulders = true; break;
      case 'upper_back': result.soreness_has_upper_back = true; break;
      case 'lower_back': result.soreness_has_lower_back = true; break;
      case 'chest': result.soreness_has_chest = true; break;
      case 'arms': result.soreness_has_arms = true; break;
      case 'wrists': result.soreness_has_wrists = true; break;
      case 'elbows': result.soreness_has_elbows = true; break;
      case 'abs_core': result.soreness_has_abs_core = true; break;
      case 'hips': result.soreness_has_hips = true; break;
      case 'glutes': result.soreness_has_glutes = true; break;
      case 'thighs': result.soreness_has_thighs = true; break;
      case 'hamstrings': result.soreness_has_hamstrings = true; break;
      case 'knees': result.soreness_has_knees = true; break;
      case 'calves': result.soreness_has_calves = true; break;
      case 'ankles': result.soreness_has_ankles = true; break;
    }
  ...
  result.soreness_has_upper_body = result.soreness_has_neck || result.soreness_has_shoulders || 
    result.soreness_has_upper_back || result.soreness_has_chest || result.soreness_has_arms || 
    result.soreness_has_wrists || result.soreness_has_elbows;
    
  result.soreness_has_core = result.soreness_has_abs_core || result.soreness_has_lower_back;
  
  result.soreness_has_lower_body = result.soreness_has_hips || result.soreness_has_glutes || 
    result.soreness_has_thighs || result.soreness_has_hamstrings || result.soreness_has_knees || 
    result.soreness_has_calves || result.soreness_has_ankles;
    
  result.soreness_has_back_issues = result.soreness_has_upper_back || result.soreness_has_lower_back;
  result.soreness_has_joint_issues = result.soreness_has_wrists || result.soreness_has_elbows || 
    result.soreness_has_knees || result.soreness_has_ankles;
  ...
  result.soreness_has_mild_only = ratings.length > 0 && ratings.every(r => r === 1);
  ...
  result.soreness_affects_cardio = result.soreness_has_lower_body || result.soreness_has_joint_issues;
  result.soreness_affects_flexibility = ratings.some(r => r >= 3);
  
  // Calculate workout modification needs
  result.soreness_needs_upper_modification = result.soreness_has_upper_body && ratings.some(r => r >= 3);
  result.soreness_needs_lower_modification = result.soreness_has_lower_body && ratings.some(r => r >= 3);
  result.soreness_needs_core_modification = result.soreness_has_core && ratings.some(r => r >= 3);
  result.soreness_needs_intensity_reduction = ratings.some(r => r >= 4);
  result.soreness_needs_recovery_focus = ratings.some(r => r >= 3);
  result.soreness_contraindicated_high_impact = result.soreness_has_joint_issues && ratings.some(r => r >= 4);
  ...
  result.soreness_average_level = ratings.length > 0 ? 
    Math.round((ratings.reduce((sum, r) => sum + r, 0) / ratings.length) * 10) / 10 : 0;
  
  // Calculate severity score (0-100)
  result.soreness_severity_score = ratings.length > 0 ? 
    Math.round((ratings.reduce((sum, r) => sum + (r * 20), 0) / ratings.length)) : 0;
  
  // Calculate recovery recommendations
  result.soreness_recommend_rest = result.soreness_severity_score >= 80 || severeCount > 0;
  result.soreness_recommend_active_recovery = result.soreness_severity_score >= 40 && result.soreness_severity_score < 80;
  result.soreness_recommend_mobility_work = result.soreness_has_joint_issues || result.soreness_has_back_issues;
  result.soreness_recommend_massage = result.soreness_has_upper_body || result.soreness_has_lower_body;
...
export function getSorenessSummary(flattened: EnhancedSorenessFlat): string {
  ...
  else if (flattened.soreness_has_moderate_levels) parts.push('Moderate soreness');
  ...
  else if (flattened.soreness_recommend_active_recovery) parts.push('Active Recovery');

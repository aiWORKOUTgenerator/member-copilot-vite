...
const EXERCISE_CATEGORIES = {
  "Full Body": [
    "Burpees", "Kettlebell Swings", "Thrusters", "Clean & Press",
    "Wall Balls", "Man Makers", "Jumping Jacks", "Bear Crawl"
  ],
  "Lower Body": [
    "Bodyweight Squats", "Lunges", "Step-Ups", "Glute Bridges",
    "Deadlifts (Dumbbell/Barbell)", "Wall Sit", "Jump Squats", "Bulgarian Split Squats"
  ],
  "Upper Body": [
    "Push-Ups", "Pull-Ups or Assisted Pull-Ups", "Bent-Over Rows", "Overhead Press",
    "Dumbbell Chest Press", "Renegade Rows", "Bicep Curls", "Tricep Dips"
  ],
  "Core & Stability": [
    "Plank", "Side Plank", "Mountain Climbers", "Dead Bug",
    "Bird Dog", "Leg Raises", "Russian Twists", "V-Ups"
  ],
  "Mobility & Corrective / Recovery": [
    "World's Greatest Stretch", "Cat-Cow", "90/90 Hip Stretch", "Band Pull-Aparts",
    "Foam Rolling (Quads)", "Thread the Needle", "Wall Angels", "Shoulder CARs"
  ]
};
...
const EXERCISE_CLASSIFICATIONS = {
  strength: [
    "deadlifts", "squats", "press", "rows", "curls", "dips", "pull-ups"
  ],
  cardio: [
    "burpees", "jumping jacks", "mountain climbers", "jump squats", "wall balls"
  ],
  mobility: [
    "stretch", "foam rolling", "car", "cat-cow", "thread the needle", "wall angels"
  ],
  core: [
    "plank", "dead bug", "bird dog", "leg raises", "russian twists", "v-ups"
  ],
  compound: [
    "burpees", "thrusters", "clean & press", "man makers", "deadlifts", "squats"
  ],
  isolation: [
    "bicep curls", "tricep dips", "leg raises", "wall sit"
  ],
  bodyweight: [
    "push-ups", "pull-ups", "squats", "lunges", "plank", "burpees", "jumping jacks"
  ],
  weighted: [
    "deadlifts", "kettlebell", "dumbbell", "barbell", "wall balls"
  ],
  explosive: [
    "jump squats", "burpees", "kettlebell swings", "wall balls", "jumping jacks"
  ],
  endurance: [
    "plank", "wall sit", "bear crawl", "mountain climbers"
  ]
};
...
function exerciseMatchesClassification(exercise: string, classification: string[]): boolean {
...
export function flattenIncludeExercisesData(data: string | undefined): EnhancedIncludeExercisesFlat {
  ...
  Object.entries(EXERCISE_CATEGORIES).forEach(([category, exercises]) => {
    ...
    switch (category) {
      case "Full Body":
        ...
        result.exercises_includes_burpees = checkExerciseIncluded("burpees", allExercises);
        result.exercises_includes_kettlebell_swings = checkExerciseIncluded("kettlebell swings", allExercises);
        result.exercises_includes_thrusters = checkExerciseIncluded("thrusters", allExercises);
        result.exercises_includes_clean_press = checkExerciseIncluded("clean & press", allExercises);
        result.exercises_includes_wall_balls = checkExerciseIncluded("wall balls", allExercises);
        result.exercises_includes_man_makers = checkExerciseIncluded("man makers", allExercises);
        result.exercises_includes_jumping_jacks = checkExerciseIncluded("jumping jacks", allExercises);
        result.exercises_includes_bear_crawl = checkExerciseIncluded("bear crawl", allExercises);
        break;
      case "Lower Body":
        result.exercises_has_lower_body = categoryHasExercises;
        result.exercises_includes_squats = checkExerciseIncluded("squats", allExercises);
        result.exercises_includes_lunges = checkExerciseIncluded("lunges", allExercises);
        result.exercises_includes_step_ups = checkExerciseIncluded("step-ups", allExercises);
        result.exercises_includes_glute_bridges = checkExerciseIncluded("glute bridges", allExercises);
        result.exercises_includes_deadlifts = checkExerciseIncluded("deadlifts", allExercises);
        result.exercises_includes_wall_sit = checkExerciseIncluded("wall sit", allExercises);
        result.exercises_includes_jump_squats = checkExerciseIncluded("jump squats", allExercises);
        result.exercises_includes_bulgarian_splits = checkExerciseIncluded("bulgarian split squats", allExercises);
        break;
      case "Upper Body":
        result.exercises_has_upper_body = categoryHasExercises;
        result.exercises_includes_push_ups = checkExerciseIncluded("push-ups", allExercises);
        result.exercises_includes_pull_ups = checkExerciseIncluded("pull-ups", allExercises);
        result.exercises_includes_bent_rows = checkExerciseIncluded("bent-over rows", allExercises);
        result.exercises_includes_overhead_press = checkExerciseIncluded("overhead press", allExercises);
        result.exercises_includes_chest_press = checkExerciseIncluded("chest press", allExercises);
        result.exercises_includes_renegade_rows = checkExerciseIncluded("renegade rows", allExercises);
        result.exercises_includes_bicep_curls = checkExerciseIncluded("bicep curls", allExercises);
        result.exercises_includes_tricep_dips = checkExerciseIncluded("tricep dips", allExercises);
        break;
      case "Core & Stability":
        result.exercises_has_core_stability = categoryHasExercises;
        result.exercises_includes_plank = checkExerciseIncluded("plank", allExercises);
        result.exercises_includes_side_plank = checkExerciseIncluded("side plank", allExercises);
        result.exercises_includes_mountain_climbers = checkExerciseIncluded("mountain climbers", allExercises);
        result.exercises_includes_dead_bug = checkExerciseIncluded("dead bug", allExercises);
        result.exercises_includes_bird_dog = checkExerciseIncluded("bird dog", allExercises);
        result.exercises_includes_leg_raises = checkExerciseIncluded("leg raises", allExercises);
        result.exercises_includes_russian_twists = checkExerciseIncluded("russian twists", allExercises);
        result.exercises_includes_v_ups = checkExerciseIncluded("v-ups", allExercises);
        break;
      case "Mobility & Corrective / Recovery":
        result.exercises_has_mobility_recovery = categoryHasExercises;
        result.exercises_includes_greatest_stretch = checkExerciseIncluded("world's greatest stretch", allExercises);
        result.exercises_includes_cat_cow = checkExerciseIncluded("cat-cow", allExercises);
        result.exercises_includes_hip_stretch = checkExerciseIncluded("90/90 hip stretch", allExercises);
        result.exercises_includes_band_pull_aparts = checkExerciseIncluded("band pull-aparts", allExercises);
        result.exercises_includes_foam_rolling = checkExerciseIncluded("foam rolling", allExercises);
        result.exercises_includes_thread_needle = checkExerciseIncluded("thread the needle", allExercises);
        result.exercises_includes_wall_angels = checkExerciseIncluded("wall angels", allExercises);
        result.exercises_includes_shoulder_cars = checkExerciseIncluded("shoulder cars", allExercises);
        break;
    }
  });
  ...
  const strengthCount = allExercises.filter(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.strength)).length;
  const cardioCount = allExercises.filter(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.cardio)).length;
  const mobilityCount = allExercises.filter(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.mobility)).length;
  
  result.exercises_strength_focused = strengthCount > cardioCount && strengthCount > mobilityCount;
  result.exercises_cardio_focused = cardioCount > strengthCount && cardioCount > mobilityCount;
  result.exercises_mobility_focused = mobilityCount > strengthCount && mobilityCount > cardioCount;
  result.exercises_core_focused = allExercises.some(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.core));
  
  // Calculate percentages
  if (allExercises.length > 0) {
    result.exercises_strength_percentage = Math.round((strengthCount / allExercises.length) * 100);
    result.exercises_cardio_percentage = Math.round((cardioCount / allExercises.length) * 100);
    result.exercises_mobility_percentage = Math.round((mobilityCount / allExercises.length) * 100);
  }
  
  // Analyze movement patterns
  result.exercises_compound_movements = allExercises.some(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.compound));
  result.exercises_isolation_movements = allExercises.some(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.isolation));
  result.exercises_bodyweight_only = allExercises.every(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.bodyweight));
  result.exercises_requires_weights = allExercises.some(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.weighted));
  result.exercises_explosive_power = allExercises.some(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.explosive));
  result.exercises_endurance_based = allExercises.some(ex => exerciseMatchesClassification(ex, EXERCISE_CLASSIFICATIONS.endurance));
  
  // Analyze body regions
  result.exercises_targets_lower_body = result.exercises_has_lower_body || allExercises.some(ex => ex.toLowerCase().includes('squat') || ex.toLowerCase().includes('lunge'));
  result.exercises_targets_upper_body = result.exercises_has_upper_body || allExercises.some(ex => ex.toLowerCase().includes('push') || ex.toLowerCase().includes('pull'));
  result.exercises_targets_core = result.exercises_has_core_stability || allExercises.some(ex => ex.toLowerCase().includes('plank') || ex.toLowerCase().includes('core'));
  result.exercises_targets_full_body = result.exercises_has_full_body || allExercises.some(ex => ex.toLowerCase().includes('burpee') || ex.toLowerCase().includes('thruster'));
  
  // Analyze equipment requirements
  result.exercises_requires_no_equipment = !result.exercises_requires_weights;
  result.exercises_requires_dumbbells = allExercises.some(ex => ex.toLowerCase().includes('dumbbell'));
  result.exercises_requires_kettlebells = allExercises.some(ex => ex.toLowerCase().includes('kettlebell'));
  result.exercises_requires_barbell = allExercises.some(ex => ex.toLowerCase().includes('barbell'));
  result.exercises_requires_resistance_bands = allExercises.some(ex => ex.toLowerCase().includes('band'));
  result.exercises_requires_foam_roller = allExercises.some(ex => ex.toLowerCase().includes('foam rolling'));
  result.exercises_requires_pull_up_bar = allExercises.some(ex => ex.toLowerCase().includes('pull-up'));
  result.exercises_requires_wall_space = allExercises.some(ex => ex.toLowerCase().includes('wall'));
  result.exercises_requires_floor_space = allExercises.some(ex => ex.toLowerCase().includes('plank') || ex.toLowerCase().includes('stretch'));
  ...
  allExercises.forEach(exercise => {
    ...
    // Complexity scoring
    if (EXERCISE_CLASSIFICATIONS.compound.some(term => normalized.includes(term))) complexityScore += 3;
    else if (EXERCISE_CLASSIFICATIONS.isolation.some(term => normalized.includes(term))) complexityScore += 1;
    else complexityScore += 2;
    
    // Intensity scoring
    if (EXERCISE_CLASSIFICATIONS.explosive.some(term => normalized.includes(term))) intensityScore += 4;
    else if (EXERCISE_CLASSIFICATIONS.cardio.some(term => normalized.includes(term))) intensityScore += 3;
    else if (EXERCISE_CLASSIFICATIONS.strength.some(term => normalized.includes(term))) intensityScore += 3;
    else if (EXERCISE_CLASSIFICATIONS.mobility.some(term => normalized.includes(term))) intensityScore += 1;
    else intensityScore += 2;
  });
  
  result.exercises_complexity_score = allExercises.length > 0 ? Math.round(complexityScore / allExercises.length) : 0;
  result.exercises_intensity_score = allExercises.length > 0 ? Math.round(intensityScore / allExercises.length) : 0;
  ...
  result.exercises_beginner_friendly = result.exercises_complexity_score <= 2 && !result.exercises_high_intensity;
  result.exercises_intermediate_level = result.exercises_complexity_score === 2 || result.exercises_moderate_intensity;
  result.exercises_advanced_level = result.exercises_complexity_score >= 3 || result.exercises_high_intensity;
  
  // Set recommendations based on exercise characteristics
  result.exercises_recommend_warmup = result.exercises_high_intensity || result.exercises_explosive_power;
  result.exercises_recommend_cooldown = result.exercises_high_intensity || result.exercises_mobility_focused;
  result.exercises_recommend_progression = result.exercises_advanced_level;
  result.exercises_recommend_supervision = result.exercises_requires_weights || result.exercises_advanced_level;
  result.exercises_recommend_form_emphasis = result.exercises_compound_movements || result.exercises_strength_focused;
...
function checkExerciseIncluded(exerciseName: string, allExercises: string[]): boolean {
...
export function getIncludeExercisesSummary(flattened: EnhancedIncludeExercisesFlat): string {
  const parts = [];
  ...
  else if (flattened.exercises_intermediate_level) parts.push('Intermediate level');
  else if (flattened.exercises_beginner_friendly) parts.push('Beginner-friendly');
  
  if (flattened.exercises_high_intensity) parts.push('High intensity');
  else if (flattened.exercises_moderate_intensity) parts.push('Moderate intensity');
  else if (flattened.exercises_low_intensity) parts.push('Low intensity');
  
  // Equipment requirements
  if (flattened.exercises_requires_no_equipment) parts.push('No equipment needed');

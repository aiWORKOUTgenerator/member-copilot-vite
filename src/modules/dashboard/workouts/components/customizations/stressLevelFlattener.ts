...
export function flattenStressLevelData(data: CategoryRatingData | undefined): EnhancedStressLevelFlat {
  ...
  const stressCategories = Object.keys(data).filter(key => data[key].selected);
  ...
  stressCategories.forEach(category => {
    ...
    // Set presence flags
    switch (category) {
      case 'physical': result.stress_has_physical = true; break;
      case 'mental_emotional': result.stress_has_mental_emotional = true; break;
      case 'environmental': result.stress_has_environmental = true; break;
      case 'systemic': result.stress_has_systemic = true; break;
    }
    ...
    if (rating) {
      ...
      switch (category) {
        ...
        case 'mental_emotional':
          ...
          else if (rating === 2) result.stress_mental_emotional_low_moderate = true;
        ...
        case 'environmental':
          ...
          else if (rating === 2) result.stress_environmental_low_moderate = true;
  ...
  result.stress_primarily_physical = physicalRating > 0 && physicalRating >= Math.max(mentalRating, environmentalRating, systemicRating);
  result.stress_primarily_mental = mentalRating > 0 && mentalRating >= Math.max(physicalRating, environmentalRating, systemicRating);
  result.stress_mixed_sources = stressCategories.length >= 2 && !result.stress_primarily_physical && !result.stress_primarily_mental;
  ...
  result.stress_has_mild_only = ratings.length > 0 && ratings.every(r => r === 1);
  ...
  result.stress_escalating_pattern = stressCategories.length >= 2 && ratings.some(r => r >= 4);
  
  // Calculate training impact assessment
  result.stress_affects_physical_performance = result.stress_has_physical && physicalRating >= 3;
  result.stress_affects_mental_focus = result.stress_has_mental_emotional && mentalRating >= 3;
  result.stress_affects_consistency = result.stress_has_environmental && environmentalRating >= 3;
  result.stress_affects_recovery = result.stress_has_systemic && systemicRating >= 3;
  result.stress_affects_motivation = (mentalRating >= 3 && systemicRating >= 2) || (mentalRating >= 4) || (systemicRating >= 4);
  result.stress_affects_exercise_selection = (environmentalRating >= 3 && physicalRating >= 2) || environmentalRating >= 4;
  
  // Calculate workout modification flags
  result.stress_needs_intensity_reduction = physicalRating >= 4 || systemicRating >= 4 || ratings.some(r => r === 5);
  result.stress_needs_duration_reduction = ratings.filter(r => r >= 4).length >= 2;
  result.stress_needs_complexity_reduction = mentalRating >= 4 || (mentalRating >= 3 && systemicRating >= 3);
  result.stress_needs_flexibility_increase = environmentalRating >= 3;
  result.stress_needs_recovery_emphasis = systemicRating >= 3 || ratings.filter(r => r >= 4).length >= 2;
  result.stress_contraindicated_high_intensity = severeCount > 0 || ratings.filter(r => r >= 4).length >= 3;
  ...
  result.stress_recommend_stress_management = ratings.filter(r => r >= 4).length >= 2;
  result.stress_recommend_professional_support = systemicRating === 5 || severeCount >= 2;
  
  // Calculate exercise type suitability
  const overallStressLevel = ratings.length > 0 ? Math.max(...ratings) : 0;
  const averageStressLevel = ratings.length > 0 ? ratings.reduce((sum, r) => sum + r, 0) / ratings.length : 0;
  
  result.stress_suitable_strength_training = overallStressLevel <= 3 && averageStressLevel <= 2.5;
  result.stress_suitable_cardio_moderate = overallStressLevel <= 4 && !result.stress_contraindicated_high_intensity;
  result.stress_suitable_cardio_intense = overallStressLevel <= 2 && averageStressLevel <= 2;
  result.stress_suitable_yoga_meditation = mentalRating >= 2 || overallStressLevel >= 3;
  ...
  result.stress_prefer_morning_workouts = mentalRating >= 3 || systemicRating >= 3;
  result.stress_prefer_shorter_sessions = overallStressLevel >= 4 || averageStressLevel >= 3;
  result.stress_prefer_familiar_routines = mentalRating >= 3 || environmentalRating >= 3;
  result.stress_prefer_social_support = mentalRating >= 3 && mentalRating <= 4;
  result.stress_prefer_solo_activities = mentalRating === 5 || systemicRating === 5;
  ...
  result.stress_average_level = ratings.length > 0 ? 
    Math.round((ratings.reduce((sum, r) => sum + r, 0) / ratings.length) * 10) / 10 : 0;
  
  // Calculate overall load score (0-100)
  result.stress_overall_load_score = ratings.length > 0 ? 
    Math.round((ratings.reduce((sum, r) => sum + (r * 20), 0) / ratings.length)) : 0;
  
  // Calculate allostatic load (cumulative stress burden)
  result.stress_allostatic_load = Math.min(100, Math.round(
    (physicalRating * 25) + 
    (mentalRating * 25) + 
  ...
  result.stress_adaptive_training_needed = overallStressLevel >= 3 || averageStressLevel >= 2.5;
  result.stress_periodization_adjustment = systemicRating >= 4 || ratings.filter(r => r >= 4).length >= 2;
  result.stress_autoregulation_recommended = result.stress_multiple_categories && overallStressLevel >= 3;
  result.stress_external_support_needed = result.stress_recommend_professional_support;
...
export function getStressLevelSummary(flattened: EnhancedStressLevelFlat): string {
  const parts = [];
  ...
  else if (flattened.stress_primarily_physical) parts.push('Physical stress dominant');
  else if (flattened.stress_primarily_mental) parts.push('Mental stress dominant');
  else if (flattened.stress_mixed_sources) parts.push('Mixed stress sources');
  
  // Recovery capacity
  if (flattened.stress_recovery_capacity_critical) parts.push('Critical recovery impairment');
  else if (flattened.stress_recovery_capacity_low) parts.push('Low recovery capacity');
  else if (flattened.stress_recovery_capacity_moderate) parts.push('Moderate recovery capacity');
  else if (flattened.stress_recovery_capacity_high) parts.push('Good recovery capacity');
  
  // Key recommendations
  if (flattened.stress_recommend_professional_support) parts.push('Professional support needed');
  else if (flattened.stress_recommend_rest_prioritization) parts.push('Rest prioritization');
  else if (flattened.stress_recommend_stress_management) parts.push('Stress management needed');
  
  // Training adaptations
  if (flattened.stress_contraindicated_high_intensity) parts.push('Avoid high intensity');
  else if (flattened.stress_needs_intensity_reduction) parts.push('Reduce intensity');

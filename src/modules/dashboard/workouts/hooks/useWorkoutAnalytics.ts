...
export const useWorkoutAnalytics = () => {
  ...
  const trackPreferenceChange = useCallback(
    (preferenceType: string, value: unknown) => {
  ...
  const trackSummaryPageView = useCallback(
    (mode: 'custom' | 'quick', customizationCount: number) => {
  ...
  const trackGenerationSuccess = useCallback(
    (workoutId: string) => {
  ...
  const trackGenerationError = useCallback(
    (error: string) => {
